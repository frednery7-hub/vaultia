...
class SaveStorageRecordMetadataUseCase(
    private val repository: StorageRepository,
) {
    fun execute(metadata: StorageRecordMetadata): StorageUseCaseResult<StorageRecordMetadata> {
        return when (val result = repository.save(metadata)) {
